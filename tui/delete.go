...
func (a *App) showDeleteConfirm() {
	...
	deleteBtn.SetStyle(tcell.StyleDefault.Background(ColorRed).Foreground(ColorWhite))
	deleteBtn.SetActivatedStyle(tcell.StyleDefault.Background(ColorYellow).Foreground(ColorRed))

	cancelBtn := tview.NewButton("Cancel").SetSelectedFunc(func() {
		a.returnFromDelete()
	})
	cancelBtn.SetStyle(tcell.StyleDefault.Background(ColorGreen).Foreground(ColorBlack))
	cancelBtn.SetActivatedStyle(tcell.StyleDefault.Background(ColorYellow).Foreground(ColorGreen))
	...
	outerLayout := tview.NewFlex().
		...
		AddItem(nil, 0, 1, false)
	outerLayout.SetBackgroundColor(tcell.ColorDefault)
...
func (a *App) showDeleteProgress(envName string) {
	...
	outputView.SetBorder(true).
		SetTitle(fmt.Sprintf(" Deleting: %s ", envName)).
		SetTitleColor(ColorYellow)

	statusBar := tview.NewTextView().
		SetDynamicColors(true).
		SetTextAlign(tview.AlignCenter).
		SetText("[yellow]Deleting... Please wait[-]")
	...
	go func() {
		...
		a.tview.QueueUpdateDraw(func() {
			if err != nil {
				...
				statusBar.SetText("[green]Environment deleted successfully![-]")
			}
